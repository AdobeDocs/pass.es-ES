...
1. La aplicación del SP presenta un &quot;Selector de MVPD&quot; al usuario para obtener su proveedor de TV de pago (MVPD). A continuación, el SP redirige el explorador del usuario al servicio de proveedor de identidad (IdP) de MVPD seleccionado.  Este es &quot;**inicio de sesión iniciado por el programador**&quot;.  MVPD envía la respuesta del IdP al servicio de consumidor de afirmación de SAML de Adobe, donde se procesa.
...
Como se muestra en los pasos anteriores, durante el flujo de AuthN una MVPD debe aceptar una solicitud de AuthN basada en SAML y enviar una respuesta de AuthN de SAML.

La [especificación de la interfaz de autenticación y autorización de acceso al contenido en línea (OLCA)](https://www.cablelabs.com/specifications/search?query=&category=&subcat=&doctype=&content=false&archives=false){target=_blanck} presenta una solicitud y una respuesta AuthN estándar. Aunque la autenticación de Adobe Pass no requiere que las MVPD basen su mensajería de asignación de derechos en este estándar, mirar la especificación puede proporcionar a insight los atributos clave necesarios para una transacción AuthN.

>[!NOTE]
>
>La solicitud AuthN que recibe un MVPD con la autenticación de Adobe Pass no contiene una firma digital. Sin embargo, el ejemplo siguiente no muestra una firma por motivos de brevedad. Para ver un ejemplo que muestra una firma digital, vea el ejemplo de [la respuesta de autenticación](#authn-response) en las secciones siguientes.
...
| muestra:AuthnRequest | &lt;AuthnRequest> emitido por el proveedor de servicios al proveedor de identidad. |
|-----------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| AssertionConsumerServiceURL | Este es el extremo de Adobe que se utilizará en la respuesta posterior. Valor predeterminado: **http://sp.auth.adobe.com/sp/saml/SAMLAssertionConsumer** |
...
| ID | Un identificador para la solicitud. Consulte [SAML core 2.0-os](http://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf){target=_blank} sección 1.3.4 para obtener más información. |
...
| IssueInstant | El instante en el que se emite la respuesta. El valor de tiempo está codificado en UTC como se describe en [SAML core 2.0-os](http://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf){target=_blank} Sección 1.3.3. |
| ProtocolBinding | Referencia de URI que identifica un enlace de protocolo SAML que se utilizará al devolver el mensaje &lt;Response>. Consulte [SAMLBind] para obtener más información sobre los enlaces de protocolo y las referencias de URI definidas para ellos. Valor predeterminado: urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST |
...
| ds:Signature | Una firma XML que protege la integridad de y autentica al emisor de la afirmación, tal como se describe en la sección 5 de SAML core 2.0-os |
| muestra:NameIDPolicy | Especifica restricciones en el identificador de nombre que se va a utilizar para representar el asunto solicitado. |
...
| Formato | Especifica la referencia URI correspondiente a un formato de identificador de nombre Predeterminado: urn:oasis:names:tc:SAML:2.0:nameid-format:transient Adobe recomienda: urn:oasis:names:tc:SAML:2.0:nameid-format:persistent |
...
Una vez recibida y gestionada la solicitud de autenticación, MVPD debe enviar una respuesta de autenticación.
...
En el ejemplo anterior, el SP de Adobe espera recuperar el ID de usuario del Subject/NameId. Adobe SP se puede configurar para obtener el ID de usuario a partir de un atributo definido personalizado; la respuesta debe contener un elemento como el siguiente:
...
| muestra:Response | Respuesta recibida por la autenticación de Adobe Pass. |
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Destino | Una referencia de URI que indica la dirección a la que se ha enviado esta solicitud. Esto resulta útil para evitar el reenvío malintencionado de solicitudes a destinatarios no deseados, una protección que requieren algunos enlaces de protocolo. Si está presente, el destinatario real DEBE comprobar que la referencia URI identifica la ubicación en la que se recibió el mensaje. Si no es así, la solicitud DEBE descartarse. Algunos enlaces de protocolo pueden requerir el uso de este atributo. |
...
| saml:Issuer | Identifica la entidad que generó el mensaje de solicitud. (Para más información sobre este elemento, véase la sección 2.2.5. SAML core 2.0-os ) |
| muestra:Status | Un código que representa el estado de la solicitud correspondiente. |
| muestra:StatusCode | Un código que representa el estado de la actividad realizada en respuesta a la solicitud correspondiente. |
| saml:Assertion | Este tipo especifica la información básica común a todas las aserciones. |
...
| ds:DigestValue | DigestValue es un elemento que contiene el valor codificado del compendio. El resumen siempre se codifica con base64. Consulte Sintaxis y procesamiento de la firma XML |
...
| ds:X509Data | Un elemento de datos X509Data de KeyInfo contiene uno o varios identificadores de claves o certificados X509. Consulte Sintaxis y procesamiento de la firma XML |
| ds: X509Certificate | El elemento X509Certificate, que contiene un certificado [X509v3] con codificación base64 |
| saml:Subject | El asunto de las declaraciones de la afirmación. |
...
| saml:SubjectConfirmationData | Información de confirmación adicional que se utilizará en un método de confirmación específico. Por ejemplo, el contenido típico de este elemento podría ser un elemento <!--<ds:KeyInfo>--> tal como se define en la Sintaxis de la firma XML y la especificación de procesamiento |
...
| saml:Conditions | El elemento &lt;Condition> sirve como punto de extensión para nuevas condiciones. |
...
| saml:Audience | Referencia URI que identifica una audiencia a la que va dirigido. |
