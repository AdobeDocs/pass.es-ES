...
El aplicación cliente debe recuperar la configuración solo cuando el usuario necesite seleccionar su MVPD para autenticarse o volver a autenticarse.

El aplicación cliente puede usar la respuesta de configuración para presentar un selector de IU con opciones de MVPD disponibles siempre que el usuario necesite seleccionar su proveedor de TV.

El aplicación cliente debe tienda el identificador MVPD seleccionado del usuario, según lo especificado por el atributo de nivel `id` de configuración del MVPD, para continuar con las fases Authentication, Preautorización, Autorización o Cierre de sesión.
...
#### 4. ¿Debe el cliente almacenar en caché la información de respuesta de configuración de almacenamiento aplicación de forma persistente? {#configuration-phase-faq4}

El aplicación cliente debe recuperar la configuración solo cuando el usuario necesite seleccionar su MVPD para autenticarse o volver a autenticarse.

El aplicación cliente debe almacenar en caché la información de respuesta de configuración en un almacenamiento de memoria para evitar solicitudes innecesarias y mejorar la experiencia del usuario cuando:

* El usuario ya está autenticado.
* Al usuario se le ofrece acceso temporal a través de la función básica o promocional [TempPass](/help/authentication/integration-guide-programmers/features-premium/temporary-access/temp-pass-feature.md) .
* La autenticación usuario ha caducado, pero el aplicación cliente ha almacenado en caché el MVPD previamente seleccionado como una opción experiencia del usuario motivada, y simplemente solicita al usuario que confirme que todavía es suscriptor de ese MVPD.

#### 5. ¿Puede el cliente aplicación administrar su propio lista de MVPD? {#configuration-phase-faq5}
...
#### 6. ¿Puede el cliente aplicación filtrar el lista de los MVPD? {#configuration-phase-faq6}

El aplicación cliente puede filtrar el lista de MVPD proporcionados en la respuesta de configuración implementando un mecanismo personalizado basado en sus propias lógica empresarial y requisitos, como usuario ubicación o usuario historial de selección anterior.

El aplicación cliente puede filtrar el lista de los MVPD de [TempPass](/help/authentication/integration-guide-programmers/features-premium/temporary-access/temp-pass-feature.md) o los MVPD que tienen su integración aún en desarrollo o prueba.

#### 7. ¿Qué sucede si la integración con un MVPD está deshabilitada y marcada como inactiva? {#configuration-phase-faq7}

Cuando la integración con un MVPD está deshabilitada y marcada como inactiva, entonces el MVPD se elimina del lista de MVPD proporcionado en respuestas de configuración adicionales, y hay dos consecuencias importantes a considerar:

* Los usuarios no autenticados de ese MVPD ya no podrán completar la fase de Authentication usando ese MVPD.
...
* Los usuarios autenticados de ese MVPD podrán volver a completar las fases de preautorización, autorización o cierre de sesión utilizando ese MVPD.

#### 9. ¿Cómo habilitar o deshabilitar la integración con un MVPD? {#configuration-phase-faq9}

Esta operación puede ser completada a través del panel de control](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md#tve-dashboard) de Adobe Pass [de TVE por uno de los administradores de su organización o por un Adobe Pass Authentication representante que actúe en su nombre.
...
El propósito de la Fase Authentication es proporcionar al cliente aplicación la capacidad de verificar la identidad del usuario y obtener usuario metadatos información.

La fase Authentication actúa como un paso previo para la fase de autorización previa o fase de autorización cuando el cliente aplicación necesita jugar contenido.
...
El cliente aplicación el manejo de errores requiere manejar los códigos de [error](/help/authentication/integration-guide-programmers/features-standard/error-reporting/enhanced-error-codes.md#enhanced-error-codes-lists-rest-api-v2) (por ejemplo, `authenticated_profile_missing`, `authenticated_profile_expired`,  `authenticated_profile_invalidated`, etc.), que indican que el cliente aplicación requiere el usuario para autenticarse.

#### 3. ¿Qué es una sesión de autenticación y cuánto tiempo es válida? {#authentication-phase-faq3}

La sesión de autenticación es un término definido en la documentación del [glosario](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md#session) .

La sesión de autenticación almacena información sobre el proceso de autenticación iniciado que se puede recuperar del extremo de las sesiones.

La sesión de autenticación es válida para un periodo de tiempo limitado y corto especificado en el momento de la emisión por la `notAfter` marca de tiempo, lo que indica la cantidad de tiempo que el usuario tiene para completar el proceso de autenticación antes de requerir reiniciar el flujo.

El aplicación cliente puede utilizar la respuesta de la sesión de autenticación para saber cómo proceder con el proceso de autenticación. Tenga en cuenta que hay casos en los que no se requiere la autenticación del usuario, como proporcionar acceso temporal, acceso degradado o cuando el usuario ya está autenticado.

Para obtener más información, consulte los siguientes documentos:

* [API de sesión de autenticación de Crear](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-create-authentication-session.md)
* [Reanudar la sesión de autenticación API](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-resume-authentication-session.md)
* [Flujo de autenticación básico realizado dentro de la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-primary-application-flow.md)
* [Flujo de autenticación básico realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-secondary-application-flow.md)

#### 4. ¿Qué es un código de autenticación y cuánto tiempo es válido? {#authentication-phase-faq4}
...
El aplicación cliente también puede utilizar el código de autenticación para permitir que el usuario complete o reanude el proceso de autenticación, ya sea en el mismo dispositivos o en uno segundo (pantalla), teniendo en cuenta que la sesión de autenticación no expiró.
...
| [Extremo de perfiles para la API de código específica (autenticación)](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | Recupere el perfil de usuario asociado a un código de autenticación específico. | **El usuario inicia el proceso de autenticación**<br/><br/> En este escenario, la aplicación cliente debe determinar si el usuario ha completado correctamente la autenticación y recuperar su información de perfil.<br/><br/>Como resultado, inicio un mecanismo de sondeo para recuperar los perfil del usuario asociados con el código de autenticación. |

Para obtener más información, consulte Flujo de perfiles básicos dentro de aplicación](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md) primario y [Flujo de perfiles básicos realizado dentro de documentos [de aplicación](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md) secundaria.

El extremo de SSO de perfiles tiene un propósito diferente, proporciona al cliente aplicación la capacidad de crear una perfil usuario utilizando la respuesta de autenticación socio y recuperarla en una sola operación única.

| API | Descripción | Ejemplo de uso |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [API de extremo de SSO de perfiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/partner-single-sign-on-apis/rest-api-v2-partner-single-sign-on-apis-retrieve-profile-using-partner-authentication-response.md) | Crear y recuperar usuario perfil utilizando socio respuesta de autenticación. | **El usuario permite que el aplicación utilice socio inicio de sesión único para autenticarse**<br/><br/> En este escenario, el aplicación cliente debe crear un perfil usuario después de recibir la respuesta de autenticación socio y recuperarla en una sola operación única. |

Para cualquier consulta posterior, los puntos finales básicos de perfiles deben usarse para determinar el estado de autenticación del usuario, acceder usuario metadatos información, encontrar el método utilizado para autenticarse o la entidad utilizada para proporcionar identidad.

Para obtener más información, consulte la [documentos Inicio de sesión único mediante flujos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/single-sign-on-access-flows/rest-api-v2-single-sign-on-partner-flows.md) de socio y [Apple SSO Cookbook (REST API V2).](/help/authentication/integration-guide-programmers/features-standard/sso-access/partner-sso/apple-sso/apple-sso-cookbook-rest-api-v2.md)

#### 11. ¿Qué debe hacer el aplicación cliente si el usuario tiene varios perfiles MVPD? {#authentication-phase-faq11}

La decisión de admitir varios perfiles depende de las necesidades empresariales del aplicación cliente.

La mayoría de los usuarios tendrán solo una perfil. Sin embargo, en los casos en que existen múltiples perfiles (como se detalla a continuación), el aplicación cliente es responsable de determinar la mejor experiencia del usuario para perfil selección.

El cliente aplicación puede optar por solicitar al usuario que seleccione el perfil MVPD deseado o realizar la selección automáticamente, como elegir el primer usuario perfil de la respuesta o el que tenga el período de validez más largo.

La API de REST v2 admite varios perfiles para acomodar:
...
* Usuarios con suscripción MVPD combinada con servicios directos al consumidor (DTC).
* Usuarios con varias suscripciones a MVPD.

#### 12. ¿Qué ocurre cuando caducan usuario perfiles? {#authentication-phase-faq12}

Cuando usuario perfiles caducan, dejan de incluirse en la respuesta del extremo Perfiles.

Si el extremo de perfiles devuelve una respuesta de mapa de perfiles vacía, el aplicación cliente debe crear una nueva sesión de autenticación y solicitar al usuario que vuelva a autenticarse.

Para obtener más información, consulte la documentación de la API](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-create-authentication-session.md) de sesión de [autenticación Crear.
...
* Cuando el aplicación cliente revoca o actualiza la instrucción de software utilizada para obtener credenciales de cliente.

#### 14. ¿Cuándo debe el cliente aplicación inicio el mecanismo de sondeo? {#authentication-phase-faq14}

Para garantizar la eficiencia y evitar solicitudes innecesarias, el cliente aplicación debe inicio el mecanismo de sondeo en las siguientes condiciones:
...
#### 15. ¿Cuándo debe el cliente aplicación detener el mecanismo de sondeo? {#authentication-phase-faq15}

Para garantizar la eficacia y evitar solicitudes innecesarias, el cliente aplicación debe detener el mecanismo de sondeo en las siguientes condiciones:
...
|----------------------------------------------------------------------|----------------------------------------------------------------------|
| La aplicación principal (streaming) debe sondear cada 3-5 segundos. | La aplicación principal (streaming) debe sondear cada 3-5 segundos. |
...
* [API de recuperación de decisiones previas a la autorización](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-preauthorization-decisions-using-specific-mvpd.md)
...
#### 7. ¿Se puede omitir la fase de autorización si ya existe una decisión de autorización previa? {#preauthorization-phase-faq7}
...
#### 3. ¿Qué es una decisión de autorización y por cuánto tiempo es válida? {#authorization-phase-faq3}

La autorización es un término definido en la documentación del [Glosario](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md#authorization) , mientras que el término de decisión también se puede encontrar en el [Glosario](rest-api-v2-glossary.md#decision).
...
El aplicación cliente puede usar la decisión de autorización que contiene un token de medios para reproducir el flujo de recursos en caso de que la decisión (autorizada) del proveedor de TV permita al usuario acceder a él.

Para obtener más detalles, consulte los siguientes documentos:

* [API de decisiones de autorización de recuperación](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-authorization-decisions-using-specific-mvpd.md)
* [Flujo de autorización básico realizado dentro de la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authorization-primary-application-flow.md)

La decisión de autorización es válida por un periodo de tiempo limitado y corto especificado en el momento de la emisión, indicando la cantidad de tiempo que será almacenada en caché por Adobe Pass Authentication antes de requerir consulta el MVPD nuevamente.

Este periodo de tiempo limitado conocido como TTL](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md#ttl) de autorización (authZ) [puede ser visto y modificado a través del panel de control](rest-api-v2-glossary.md#tve-dashboard) de Adobe Pass [de TVE por uno de los administradores de su organización o por un Adobe Pass Authentication representante que actúe en su nombre.

Para obtener más detalles, consulte la documentación de [Guía del usuario](/help/authentication/user-guide-tve-dashboard/tve-dashboard-integrations.md#most-used-flows) Integraciones del panel de control de Televisión.

#### 4. ¿Debería el cliente almacenar en caché las decisiones de autorización de forma persistente almacenamiento aplicación? {#authorization-phase-faq4}
...
El aplicación cliente puede determinar el motivo de una decisión de autorización denegada inspeccionando el código de error y el [mensaje](/help/authentication/integration-guide-programmers/features-standard/error-reporting/enhanced-error-codes.md) incluidos en la respuesta del extremo de autorización de decisiones. Estos detalles proporcionan a insight el motivo específico por el que se denegó la solicitud de autorización, lo que ayuda a informar a la experiencia del usuario o al déclencheur de cualquier tratamiento necesario en la aplicación.

Asegúrese de que cualquier mecanismo de reintento implementado para recuperar las decisiones de autorización no resulte en un bucle interminable si se deniega la decisión de autorización.

Considere limitar reintentos a un número razonable y manejar las denegaciones con gracia al mostrar comentarios claros al usuario.

#### 6. ¿Qué es un token de medios y durante cuánto tiempo es válido? {#authorization-phase-faq6}

El token medios es un término definido en la documentación del [glosario](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md#media-token) .
...
La aplicación cliente debe validar el token de medios antes de iniciar la reproducción del flujo de recursos. Esta validación debe realizarse con el [Comprobador de token de medios](/help/authentication/integration-guide-programmers/features-standard/entitlements/media-tokens.md#media-token-verifier). Al verificar el `serializedToken` de la devolución `token`, el cliente aplicación ayuda a evitar el acceso no autorizado, como la extracción de flujos, y garantiza que solo los usuarios debidamente autorizados puedan reproducir el contenido.

#### 8. ¿Debería el cliente actualizar aplicación un token de medios caducado durante la reproducción? {#authorization-phase-faq8}

No.

No es necesario que el aplicación cliente actualice un token de medios caducado mientras la transmisión se esté reproduciendo activamente. Si el token de medios caduca durante la reproducción, se debe permitir que el flujo continúe ininterrumpidamente. Sin embargo, el cliente debe solicitar una nueva decisión de autorización y obtener un nuevo token de medios la próxima vez que el usuario intente reproducir el mismo recurso.
...
### Preguntas frecuentes sobre la fase de cierre de sesión {#logout-phase-faqs-general}
...
El propósito de la fase de cierre de sesión es proporcionar al cliente aplicación la capacidad de terminar el perfil autenticado del usuario dentro de Adobe Pass Authentication en usuario solicitud.

#### 2. ¿Es obligatoria la fase de cierre de sesión? {#logout-phase-faq2}

La fase de cierre de sesión es obligatoria, el cliente aplicación debe proporcionar al usuario la capacidad de cerrar sesión.
...
>[!IMPORTANT]
>
> En caso de que el aplicación cliente migre de la API de REST V1 a la API de REST V2, el aplicación del cliente puede seguir utilizando el mismo método para obtener el `Bearer` valor token de acceso que antes.
...
El [encabezado X-Device-Info](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/appendix/headers/rest-api-v2-appendix-headers-x-device-info.md) solicitud contiene la información del cliente (dispositivos, conexión y aplicación) relacionada con el dispositivos de transmisión real y se utiliza para determinar las reglas específicas de la plataforma que los MVPD pueden aplicar.

La [documentación del encabezado X-Device-Info](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/appendix/headers/rest-api-v2-appendix-headers-x-device-info.md) proporciona ejemplos de las principales plataformas sobre cómo calcular el valor, pero el aplicación cliente puede optar por utilizar un método diferente en función de sus propios lógica empresarial y requisitos.
...
Para interactuar con [la API de REST V2](https://developer.adobe.com/adobe-pass/api/rest_api_v2/interactive/), debe incluir el [encabezado Authorization](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/appendix/headers/rest-api-v2-appendix-headers-authorization.md) con un `Bearer` token de acceso obtenido a través de la [API de DCR](https://developer.adobe.com/adobe-pass/api/dcr_api/interactive/).

Para usar la API DCR,](https://developer.adobe.com/adobe-pass/api/dcr_api/interactive/) se requiere una declaración de [software con ámbito API REST V2. Para obtener más información, consulte el documento de preguntas frecuentes](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-faqs.md) sobre el [registro de cliente dinámico (DCR).

+++

## Preguntas frecuentes sobre migración {#migration-faqs}

Continuar con esta sección si está trabajando en un aplicación que necesita migrar un aplicación existente a la API de REST V2.

>[!MORELIKETHIS]
>
> * [Preguntas más frecuentes sobre el registro de clientes dinámicos (DCR)](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-faqs.md#migration-faqs)

### Preguntas más frecuentes generales sobre migración {#general-migration-faqs}

+++Preguntas frecuentes generales sobre migración

#### 1. ¿Tengo que desplegar un nuevo cliente aplicación migrarme a la API de REST V2 para todos los usuarios a la vez? {#migration-faq1}
...
En el caso del flujo de &quot;autenticación de segunda pantalla&quot;, se requiere que el aplicación cliente implemente una nueva versión que integre la API REST V2 para las [aplicaciones primarias](rest-api-v2-glossary.md#primary-application) y [secundarias](rest-api-v2-glossary.md#secondary-application) simultáneamente.

Adobe Pass Authentication no admitirá implementaciones &quot;híbridas&quot; que integren tanto la API de REST V2 como la API de REST V1/SDK entre las API y los flujos.

#### 3. ¿Se conservará la autenticación usuario al actualizar a un nuevo cliente aplicación migrar a la API de REST V2? {#migration-faq3}

No.

No se conservará la autenticación usuario obtenida en versiones anteriores de aplicación de cliente que integran la API de REST V1 o SDK.
...
Las aplicaciones cliente que migran a la API de REST V2 se benefician automáticamente de esta característica de forma predeterminada, proporcionando información de error más detallada y precisa.

Para obtener más detalles, consulte la documentación de los [códigos](/help/authentication/integration-guide-programmers/features-standard/error-reporting/enhanced-error-codes.md#enhanced-error-codes-lists-rest-api-v2) de Error mejorados.
...
El aplicación cliente puede seguir utilizando el mismo flujo para registrarse en Adobe Pass Authentication a través del proceso de [registro dinámico de clientes (DCR)](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md#dcr) y obtener un token de acceso.

Para obtener más información, consulte los siguientes documentos:

* [Información general sobre el registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-overview.md)
* [API de recuperación de credenciales de cliente](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md)
* [Recuperar API de token de acceso](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md)
* [Flujo de registro de cliente dinámico](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/flows/dynamic-client-registration-flow.md)
...
| Ámbito | API DE REST V1 | API DE REST V2 | Observaciones |
|-------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Recuperar código registro (código de autenticación) | [<br/> POST /reggie/v1/{serviceProvider}/regcode](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/registration-code-request.md) | [<br/> POST /api/v2/{serviceProvider}/sessions](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-create-authentication-session.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autenticación básico realizado dentro de la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-primary-application-flow.md)</li><li>[Flujo de autenticación básico realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-secondary-application-flow.md)</li></ul> |
| Compruebe registro código (código de autenticación) | [<br/> GET /reggie/v1/{serviceProvider}/regcode/{code}](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/return-registration-record.md) | [<br/> GET /api/v2/{serviceProvider}/sessions/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-retrieve-authentication-session-information-using-code.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autenticación básico realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-secondary-application-flow.md)</li></ul> |
| Autenticación de reanudación (MVPD) en la segunda pantalla (aplicación) | [<br/> GET /api/v1/authenticate](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/initiate-authentication.md) | [<br/> POST /api/v2/{serviceProvider}/sessions/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-resume-authentication-session.md) <br/> [<br/> GET /api/v2/authenticate/{serviceProvider}/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-perform-authentication-in-user-agent.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autenticación básico realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-secondary-application-flow.md)</li></ul> |
| Autenticación de inicio (MVPD) | [<br/> GET /api/v1/authenticate](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/initiate-authentication.md) | [GET <br/> /api/v2/authenticate/{serviceProvider}/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-perform-authentication-in-user-agent.md) | Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autenticación básico realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-primary-application-flow.md)</li><li>[Flujo de autenticación básico realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-secondary-application-flow.md)</li></ul> |
| Comprobar estado de autenticación del usuario | [GET <br/> /api/v1/checkauthn (primera pantalla)](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/check-authentication-token.md) <br/> [GET <br/> /api/v1/checkauthn (segunda pantalla)](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/check-authentication-flow-by-second-screen-web-app.md) | Utilice uno de los siguientes: <br/><br/> [GET <br/> /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | La aplicación cliente puede utilizar la respuesta de estas API para varios fines a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar perfil de usuario</li><li>Recuperar usuario información metadatos</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
| Recuperar token de autenticación de usuario (perfil) | [GET <br/> /api/v1/tokens/authn](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/retrieve-authentication-token.md) | Utilice una de las siguientes soluciones: <br/><br/> [GET <br/> /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | La aplicación cliente puede utilizar la respuesta de estas API para varios fines a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar usuario perfil</li><li>Recuperar usuario información metadatos</li></ul> <br/> Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos dentro de la aplicación primaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
| Recuperar usuario información metadatos | [GET <br/> /api/v1/tokens/usermetadata](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/user-metadata.md) | Utilice uno de los siguientes: <br/><br/> [GET <br/> /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | El aplicación cliente puede utilizar la respuesta de estas API para varios propósitos a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar perfil de usuario</li><li>Recuperar información de metadatos de usuario</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |

+++

#### Preguntas frecuentes sobre la fase de autorización previa {#preauthorization-phase-faqs-migration-rest-api-v1-to-rest-api-v2}

+++Preguntas frecuentes sobre la fase de preautorización

##### 1. ¿Cuáles son las migraciones de API de alto nivel necesarias para la fase de autorización previa? {#preauthorization-phase-v1-to-v2-faq1}

En la migración de REST API V1 a REST API V2 hay cambios de alto nivel a considerar que se presentan en la siguiente tabla:

| Ámbito | API DE REST V1 | API DE REST V2 | Observaciones |
|---------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Recuperar recursos preautorizados (decisiones de autorización previa) | [<br/> GET /api/v1/preauthorize (primera pantalla)](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/retrieve-list-of-preauthorized-resources.md) <br/> [<br/> GET /api/v1/preauthorize (segunda pantalla)](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/retrieve-list-of-preauthorized-resources-by-second-screen-web-app.md) | [<br/> POST /api/v2/{serviceProvider}/decisions/preauthorize/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-preauthorization-decisions-using-specific-mvpd.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo básico de autorización previa realizado dentro de la aplicación primaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-preauthorization-primary-application-flow.md)</li></ul> |
...
| Ámbito | API DE REST V1 | API DE REST V2 | Observaciones |
|-------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Autorización de Initiate (MVPD) | [<br/> GET /api/v1/authorize](/help/authentication/integration-guide-programmers/legacy/rest-api-v1/apis/initiate-authorization.md) | [<br/> POST /api/v2/{serviceProvider}/decisions/authorize/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-authorization-decisions-using-specific-mvpd.md) | El aplicación cliente puede utilizar la respuesta de esta API para varios propósitos a la vez: <br/> <ul><li>Iniciar autorización de (MVPD)</li><li>Recuperar decisión de autorización</li><li>Recuperar token de medios corto</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autorización básico realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authorization-primary-application-flow.md)</li></ul> |
...
En la migración de SDK a REST API V2 hay cambios de alto nivel a considerar que se presentan en las siguientes tablas:

###### AccessEnabler JavaScript SDK

| Ámbito | SDK | API DE REST V2 | Observaciones |
|--------------------------------------------|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Registro de cliente dinámico (DCR) de Todas las aplicaciones | Proporcionar una declaración de software al constructor | [<br/> POST /o/client/register](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md) <br/> [GET <br/> /o/client/token](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md) | Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Información general sobre el registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-overview.md)</li><li>[Flujo de registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/flows/dynamic-client-registration-flow.md)</li></ul> |

###### AccessEnabler iOS/tvOS SDK

| Ámbito | SDK | API DE REST V2 | Observaciones |
|--------------------------------------------|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Registro dinámico de clientes (DCR) completo | Proporcionar instrucción de software al constructor | [POST <br/> /o/client/register](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md) <br/> [GET <br/> /o/client/token](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Información general sobre el registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-overview.md)</li><li>[Flujo de registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/flows/dynamic-client-registration-flow.md)</li></ul> |

###### AccessEnabler Android SDK

| Ámbito | SDK | API DE REST V2 | Observaciones |
|--------------------------------------------|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Registro de cliente dinámico (DCR) de Todas las aplicaciones | Proporcionar instrucción de software al constructor | [POST <br/> /o/client/register](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md) <br/> [GET <br/> /o/client/token](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md) | Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Información general sobre el registro dinámico de clientes](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-overview.md)</li><li>[Flujo de registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/flows/dynamic-client-registration-flow.md)</li></ul> |

###### AccessEnabler FireOS SDK

| Ámbito | SDK | API DE REST V2 | Observaciones |
|--------------------------------------------|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Registro dinámico de clientes (DCR) completo | Proporcionar instrucción de software al constructor | [<br/> POST /o/client/register](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md) <br/> [<br/> GET /o/client/token](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Información general sobre el registro de clientes dinámicos](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-overview.md)</li><li>[Flujo de registro de cliente dinámico](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/flows/dynamic-client-registration-flow.md)</li></ul> |
...
| Recuperar información de metadatos de usuario | [AccessEnabler.getMetadata](/help/authentication/integration-guide-programmers/legacy/sdks/javascript-sdk/javascript-sdk-api-reference.md#getMeta) | Utilice uno de los siguientes: <br/><br/> [GET <br/> /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | El aplicación cliente puede utilizar la respuesta de estas API para varios propósitos a la vez: <br/> <ul><li>Comprobar usuario estado de autenticación</li><li>Recuperar usuario perfil</li><li>Recuperar usuario información metadatos</li></ul> <br/> Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos dentro de la aplicación primaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |

###### AccessEnabler iOS SDK

| Ámbito | SDK | API DE REST V2 | Observaciones |
|------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Autenticación de inicio (MVPD) | [AccessEnabler.getAuthentication](/help/authentication/integration-guide-programmers/legacy/sdks/ios-tvos-sdk/iostvos-sdk-api-reference.md#getAuthN) <br/> [AccessEnabler.setSelectedProvider](/help/authentication/integration-guide-programmers/legacy/sdks/ios-tvos-sdk/iostvos-sdk-api-reference.md#setSelProv) | [<br/> POST /api/v2/{serviceProvider}/sessions](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-create-authentication-session.md) <br/> [<br/> GET /api/v2/authenticate/{serviceProvider}/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-perform-authentication-in-user-agent.md) | Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autenticación básico realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-primary-application-flow.md)</li><li>[Flujo de autenticación básico realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authentication-secondary-application-flow.md)</li></ul> |
| Comprobar estado de autenticación del usuario | [AccessEnabler.checkAuthentication](/help/authentication/integration-guide-programmers/legacy/sdks/ios-tvos-sdk/iostvos-sdk-api-reference.md#checkAuthN) | Utilice una de las siguientes soluciones: <br/><br/> [<br/> GET /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | La aplicación cliente puede utilizar la respuesta de estas API para varios fines a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar usuario perfil</li><li>Recuperar usuario información metadatos</li></ul> <br/> Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
| Recuperar información de metadatos de usuario | [AccessEnabler.getMetadata](/help/authentication/integration-guide-programmers/legacy/sdks/ios-tvos-sdk/iostvos-sdk-api-reference.md#getMeta) | Utilice uno de los siguientes: <br/><br/> [<br/> GET /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | La aplicación cliente puede utilizar la respuesta de estas API para varios fines a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar perfil de usuario</li><li>Recuperar información de metadatos de usuario</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
...
| Recuperar información de metadatos de usuario | [AccessEnabler.getMetadata](/help/authentication/integration-guide-programmers/legacy/sdks/ios-tvos-sdk/iostvos-sdk-api-reference.md#getMeta) | Utilice uno de los siguientes: <br/><br/> [GET <br/> /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [GET <br/> /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | La aplicación cliente puede utilizar la respuesta de estas API para varios fines a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar usuario perfil</li><li>Recuperar información de metadatos de usuario</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
...
| Comprobar estado de autenticación del usuario | [AccessEnabler.checkAuthentication](/help/authentication/integration-guide-programmers/legacy/sdks/android-sdk/android-sdk-api-reference.md#checkAuthN) | Utilice uno de los siguientes: <br/><br/> [<br/> GET /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | El aplicación cliente puede utilizar la respuesta de estas API para varios propósitos a la vez: <br/> <ul><li>Comprobar usuario estado de autenticación</li><li>Recuperar usuario perfil</li><li>Recuperar usuario información metadatos</li></ul> <br/> Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos dentro de la aplicación primaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado dentro de aplicación secundarios](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
| Recuperar usuario información metadatos | [AccessEnabler.getMetadata](/help/authentication/integration-guide-programmers/legacy/sdks/android-sdk/android-sdk-api-reference.md#getMetadata) | Utilice una de las siguientes soluciones: <br/><br/> [<br/> GET /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | El aplicación cliente puede utilizar la respuesta de estas API para varios propósitos a la vez: <br/> <ul><li>Comprobar usuario estado de autenticación</li><li>Recuperar perfil de usuario</li><li>Recuperar información de metadatos de usuario</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
...
| Recuperar información de metadatos de usuario | [AccessEnabler.getMetadata](/help/authentication/integration-guide-programmers/legacy/sdks/fireos-sdk/amazon-fireos-native-client-api-reference.md#getMetadata) | Utilice una de las siguientes soluciones: <br/><br/> [<br/> GET /api/v2/{serviceProvider}/profiles](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) <br/> [<br/> GET /api/v2/{serviceProvider}/profiles/code/{code}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | El aplicación cliente puede utilizar la respuesta de estas API para varios propósitos a la vez: <br/> <ul><li>Comprobar estado de autenticación del usuario</li><li>Recuperar perfil de usuario</li><li>Recuperar información de metadatos de usuario</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de perfiles básicos realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-primary-application-flow.md)</li><li>[Flujo de perfiles básicos realizado en la aplicación secundaria](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-profiles-secondary-application-flow.md)</li></ul> |
...
| Ámbito | SDK | API DE REST V2 | Observaciones |
|---------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Recuperar recursos preautorizados (decisiones de autorización previa) | [AccessEnabler.checkPreauthorizedResources](/help/authentication/integration-guide-programmers/legacy/sdks/fireos-sdk/amazon-fireos-native-client-api-reference.md#checkPreauth) | [<br/> POST /api/v2/{serviceProvider}/decisions/preauthorize/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-preauthorization-decisions-using-specific-mvpd.md) | Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de preautorización básico realizado en la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-preauthorization-primary-application-flow.md)</li></ul> |
...
| Ámbito | SDK | API DE REST V2 | Observaciones |
|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Recuperar token de autorización corto (token de medios) | [AccessEnabler.checkAuthorization](/help/authentication/integration-guide-programmers/legacy/sdks/fireos-sdk/amazon-fireos-native-client-api-reference.md#checkAuthZ) <br/> [AccessEnabler.getAuthorization](/help/authentication/integration-guide-programmers/legacy/sdks/fireos-sdk/amazon-fireos-native-client-api-reference.md#getAuthZ) | [<br/> POST /api/v2/{serviceProvider}/decisions/authorize/{mvpd}](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-authorization-decisions-using-specific-mvpd.md) | El aplicación cliente puede utilizar la respuesta de esta API para varios propósitos a la vez: <br/> <ul><li>Autorización de Initiate (MVPD)</li><li>Recuperar decisión de autorización</li><li>Recuperar token de medios corto</li></ul> <br/> Para obtener más información, consulte los siguientes documentos: <br/> <ul><li>[Flujo de autorización básico realizado dentro de la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-authorization-primary-application-flow.md)</li></ul> |

+++

#### Preguntas frecuentes sobre la fase de cierre de sesión {#logout-phase-faqs-migration-sdk-to-rest-api-v2}
...
| Ámbito | SDK | API DE REST V2 | Observaciones |
|-----------------|----------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Iniciar cierre de sesión | [AccessEnabler.logout](/help/authentication/integration-guide-programmers/legacy/sdks/android-sdk/android-sdk-api-reference.md#logout) | [<br/> GET /api/v2/{serviceProvider}/logout](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/logout-apis/rest-api-v2-logout-apis-initiate-logout-for-specific-mvpd.md) | Para obtener más detalles, consulte los siguientes documentos: <br/> <ul><li>[Flujo de cierre de sesión básico realizado dentro de la aplicación principal](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/basic-access-flows/rest-api-v2-basic-logout-primary-application-flow.md)</li></ul> |
