...
Los términos adicionales utilizados en el flujo se definen en el [Glosario](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/rest-api-v2-glossary.md).
...
([Recuperar lista de MVPD disponibles](../apis/configuration-apis/rest-api-v2-configuration-apis-retrieve-configuration-for-specific-service-provider.md))
   ...
   * El servicio de programación debe devolver una lista de MVPD filtrada para que la aplicación de streaming muestre el selector, el usuario selecciona el MVPD
   * Con MVPD seleccionado en la aplicación de transmisión, el servicio de programador crea una sesión: <b>/api/v2/{serviceProvider}/session</b><br>
...
* Opción 1. La aplicación de streaming puede abrir un explorador o una vista web, cargar la URL para autenticarse y el usuario aterriza en la página de inicio de sesión de MVPD donde deben enviarse las credenciales
...
([Recuperar perfiles autenticados para MVPD específico](../apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md))
   * Si la selección de MVPD no se realiza en la aplicación de streaming porque el selector de MVPD se presenta en la aplicación de segunda pantalla, el sondeo debe realizarse con CODE <b>/api/v2/{serviceProvider}/profiles/code/{CODE}</b><br>
...
([Recuperar decisión de preautorización utilizando MVPD específico](../apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-preauthorization-decisions-using-specific-mvpd.md))
...
([Recuperar decisión de autorización utilizando MVPD específico](../apis/decisions-apis/rest-api-v2-decisions-apis-retrieve-authorization-decisions-using-specific-mvpd.md))
...
Aplicación de streaming: el usuario quiere cerrar la sesión de MVPD

* La aplicación de streaming informa al servicio de programación de que debe cerrar la sesión de MVPD para esta aplicación específica.
...
([Iniciar el cierre de sesión para un MVPD específico](../apis/logout-apis/rest-api-v2-logout-apis-initiate-logout-for-specific-mvpd.md))
