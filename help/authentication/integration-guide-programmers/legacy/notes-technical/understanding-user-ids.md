...
| getMetadata(), propiedad userID | Vinculación de cuentas, investigación de fraude con MVPD | No | No | : Esta propiedad permite a Adobe exponer el ID de usuario de MVPD de origen real al programador. </br> </br>: en la configuración de Adobe, se puede establecer como cifrado o no (según la preferencia de MVPD). Si está cifrado, se cifrará con la clave pública del certificado del programador proporcionado a Adobe, de modo que no se exponga de forma clara al cliente. </br> </br>: esto proporciona al programador el ID de usuario real de MVPD, por lo que puede utilizarse para vincular cuentas o investigar fraudes directamente con MVPD. |


**En conclusión**

* En general, MVPD proporciona un identificador único persistente <u> y lo pasa a Adobe cuando la autenticación se realiza correctamente</u>. En general, es coherente en todas las redes. La excepción es Comcast MVPD, que proporciona un ID de usuario diferente para cada canal.
